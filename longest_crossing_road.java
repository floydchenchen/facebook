...
public class longest_crossing_road {
    // DFS, time: O(mn(m+n)), space: O(n)
    public static int maxXing(int[][] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            for (int j = 0; j < nums[0].length; j++) {
                ...
                max = Integer.max(max, colHit[j] + rowHit);
            }
        }
        return max;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxXing(arr));
    }
}
